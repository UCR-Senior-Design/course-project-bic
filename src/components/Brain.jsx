...
// Import all SVG files matching the naming pattern
const importAll = (context) => context.keys().map(context);
const carpetplotImages = importAll(require.context('../svg-images', false, /_task-tv_run-01_desc-carpetplot_bold\.svg$/));
...
const Brain = () => {
  return (
    <div className="mt-4">
      ...
      {carpetplotImages.map((image, index) => (
        <div key={index}>
          Subject {index + 1}
          <div className="mt-4">
            <img width="40%" className="rounded" src={image.default} alt={`brain${index + 1}`} />
          </div>
        </div>
      ))}
    </div>
